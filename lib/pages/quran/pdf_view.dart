...
class _QuranViewerScreenState extends State<QuranViewerScreen> {
  ...
  // New variables for FAB visibility control
  ...
  void initState() {
    ...
    loadPdf();
    _loadSettings();
    _startFabTimer(); // Start the timer initially
  }
  ...
  void goToPage(String pageNumber) {
    ...
    if (page != null && page >= 1 && page <= pages) {
      ...
      _startFabTimer(); // Restart timer on page change
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Please enter a valid page (1-$pages)'),
  ...
  Future<void> _saveBookmark() async {
    ...
    _startFabTimer(); // Restart timer after bookmarking
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        leading: IconButton(
          icon: Icon(Icons.arrow_back_ios_new, color: isNightMode ?  Colors.white :Colors.black,),
      ...
      body: GestureDetector(
        onTap: () {
          _startFabTimer(); // Show FAB when screen is tapped
        },
        child: Stack(
          children: [
            if (isReloading)
            ...
            Positioned(
              ...
              child: GestureDetector(
                onTap: () {
                  _saveBookmark();
                  _startFabTimer(); // Restart timer when FAB is tapped
                },
              ...
              child: FloatingActionButton(
                ...
                onPressed: currentPage > 0
                    ? () {
                        ...
                        _startFabTimer(); // Restart timer on navigation
                      }
                    : null,
              ),
            ),
